...
export class FlightService {
    ...
    async createFlightToItinerary(itinerary_id: number, createFlightDto: CreateFlightDto) {
        const itinerary = await this.itineraryService.getItineraryById(itinerary_id);
        ...
        try {
            return await this.sequelize.transaction(async (t) => {
                return this.flightModel.create({...createFlightDto, itinerary_id}, {
                    transaction: t
                });
            });
        }
        catch(e) {
            throw new HttpException('Flight Creation Failed', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    async getFlightsForItinerary(itinerary_id: number, page: number) {
    ...
    async updateFlightById(id: number, createFlightDto: CreateFlightDto) {
        try {
            const [affectedCount] = await this.sequelize.transaction(async (t) => {
                return await this.flightModel.update({...createFlightDto}, {
                    where: {
                        id 
                    },
                    transaction: t
            ...
            throw new HttpException('Flight Update Failed', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async deleteFlightById(id: number) {
        try {
            ...
            throw new HttpException('Flight Deletion Failed', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
}
